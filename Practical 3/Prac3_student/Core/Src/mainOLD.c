...
uint32_t adc_val;
uint32_t currenttick = 0;//Current tick value in ms
uint32_t previoustick = 0;//Previous tick value in ms
char sensorValue[10];//Value of ADC in char array
...
int main(void)
{
  ...
  while (1)
  {
	...
	uint32_t sensor = pollADC();// sensor is the value of the adc as an int (use this for CRR)
	sprintf(sensorValue,"%lu",sensor);// sensorValue is the value of the adc as a char array
	writeLCD(sensorValue);
...
void EXTI0_1_IRQHandler(void)
{
	...
	HAL_GPIO_EXTI_IRQHandler(adc); // Clear interrupt flags
}
...
uint32_t ADCtoCCR(uint32_t adc_val){
  // TODO: Calculate CCR val using an appropriate equation
	uint32_t val = 11.72136752*adc_val;
	return val;
}
